  ...
  function getAllUsers($page) {
   ...
   function getSearchUsers($page, $name) {
    ...
    /*$stmt = $conn->prepare('SELECT  public.Users.first_name, public.Users.last_name, public.Users.email, public.Authenticated_User.photo_url
							...
							LIMIT 10 OFFSET ? * 10;');*/
	$stmt = $conn->prepare("SELECT public.Users.first_name, public.Users.last_name, public.Users.email, public.Authenticated_User.photo_url FROM public.Authenticated_User, public.Users WHERE public.Users.first_name LIKE '%c%' LIMIT 10 OFFSET 1 * 10;");
    $stmt->execute();
    return $stmt->fetchAll();
  }
?>